...
export const AuditServices = {
    getAllAuditLog: async (query) => {
        ...
        if (query.responseStatus) filter.responseStatus = Number(query.responseStatus);

        const data = await AuditLog.find(filter).lean();
        if (!data || data.length === 0) {
            return MasterResponse({ status: STATUS.NOT_FOUND, errCode: ERROR_CODE.BAD_REQUEST, message: "No audit logs found" });
        }
    ...
    countAuditLog: async () => {
        ...
        if (!count) {
            return MasterResponse({ status: STATUS.NOT_FOUND, errCode: ERROR_CODE.BAD_REQUEST, message: "Can't get audit log" })
        }
