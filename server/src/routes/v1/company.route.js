...
Router.route('/')
    .post(checkBlackList, verifyToken, authorizeAdmin('user'), companyController.postCompany)
    .get(cacheMiddleware, companyController.getCompanies)

Router.route('/avatar')
    .post(checkBlackList, verifyToken, authorizeAdmin('user'), uploadCompanyImage.single('avatar'), companyController.uploadAvatar)
