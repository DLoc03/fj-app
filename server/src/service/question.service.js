...
const createQuestion = async (userId, jobId, question) => {
    ...
    return MasterResponse({ status: STATUS.CREATED, message: "Question was created", data: newQuestion })
}

const getQuestWithJob = async (id) => {
    const existedJob = await Job.findOne({ _id: id }).lean()
    if (!existedJob) return MasterResponse({ status: STATUS.NOT_FOUND, message: "Job not found", errCode: ERROR_CODE.BAD_REQUEST })
